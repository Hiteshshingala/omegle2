...
let roomId = window.localStorage.room; //new URLSearchParams(window.location.search).get('room');
const peerName = window.localStorage.name; //new URLSearchParams(window.location.search).get('name');
    ...
    xhr.onreadystatechange = function() {
        if (xhr.readyState == 4 && xhr.status == 200) {
            ...
            if(xhr.responseText) {
                let resp = JSON.parse(xhr.responseText);
                if(resp.roomId) {
                console.log('Join to channel', resp.roomId);
                roomId = resp.roomId;
                sendToServer('join', {
                    channel: resp.roomId,
                    peerInfo: {
...
function setLocalMedia(stream) {
    ...
    const videoElDiv = document.createElement('div');
